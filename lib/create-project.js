...
import { cp, mkdir, mkdtemp, rm, readFile, access, readdir } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join, resolve } from 'node:path';

export const repository = 'https://github.com/CaveatJS/site.git';
export const revision = 'b9ef3e61ff308189fcd7a5e577463e0a1921258a';
...
export async function createProject(directory, source = { repository, revision }) {
  ...
  try {
    ...
    await access(join(checkout, 'src', 'app', 'page.tsx'));
    await access(join(checkout, 'prisma', 'schema.prisma'));
    await access(join(checkout, 'scripts', 'dev.mjs'));
    await access(join(checkout, 'package-lock.json'));
    for (const entry of await readdir(checkout)) {
      if (entry === '.git') continue;
      await cp(join(checkout, entry), join(target, entry), {
